				...
				<tr height = "40">
					<td align ="center" colspan = "4"> 
						...
						<input type="button" value="답글쓰기" 
                 			 onclick="location.href='Question_D_ReWriteform.jsp?num=<%=vo.getNum()%>&ref=<%=vo.getRef()%>&re_stop=<%=vo.getRes()%>&re_level=<%=vo.getRel()%>'"> 
						<% } else { %>
						<input style="display: none;" type="button" value="답글쓰기" 
                 			 onclick="location.href='Question_D_ReWriteform.jsp?num=<%=vo.getNum()%>&ref=<%=vo.getRef()%>&re_stop=<%=vo.getRes()%>&re_level=<%=vo.getRel()%>'"> 
						<% } %>	
